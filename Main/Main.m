...
m=6;                                                          % number of elements in y-direction
...
dd1     =    D/2;      %-- [m] distance to the front axle from G
...
mu3      =      751;
mu4      =      469;
ms1   =     (l*dd2)/((dd1+dd2 )*(2*l))*ms;
ms2  =     (l*dd1)/((dd1+dd2 )*(2*l))*ms ; 
ms3  =      (l*dd2)/((dd1+dd2 )*(2*l))*ms ; 
ms4  =      (l*dd1)/((dd1+dd2 )*(2*l))*ms;
...
y_y= [ 0.68*ones(1,n);0.68*ones(1,n); L+0.68*ones(1,n); L+0.68*ones(1,n)]; 

x_x= [x0+v*t2+dd1; x0+v*t2-dd2 ;x0+v*t2+dd1 ;x0+v*t2-dd2]-xb0;    %-- vehicle position: x(t) [1 2 3 4] or [F R F R]
...
D0=E*thick^3/(12*(1-nu^2));
[Kb,Mb]= M_K_mat(NoE,NPE,PD,NL,EL,D0,rhA_data);
...
%      Fb(:,end)=0;
     ...
     bc1= find(NL(:,1)==0|NL(:,1)==d1); %Pinned pinned
%      bc2=find(NL(:,1)==d1);
    nn1= length(bc1); % how many nodes have x=0 or d1 as entry
     bc_Dof=[];
     bc_dof=zeros(nn1,1);
     for i= 1:nn1
         bc_Dof=[bc_Dof;3*bc1(i)-2;3*bc1(i)-1];
